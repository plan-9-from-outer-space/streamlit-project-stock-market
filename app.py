...
@st.cache_data
def company_price(df_sp, company_name):
    if company_name != None:
        ticker_symbol = df_sp.loc[df_sp['name'] == company_name, 'ticker'].values[0]
        df = pdr.get_data_yahoo(ticker_symbol, start="2013-09-30", end="2023-09-30")['Adj Close']
...
def show_stock_price(data_price):
    fig = px.line(data_price, x="ds", y="y", title='10 Year Stock Price')
...
def metrics(data_price):
    stock_price_start = data_price['y'].values[0] # Take the first value
    stock_price_end = data_price['y'].values[-1]  # Take the last value
    performance = np.around((stock_price_end / stock_price_start - 1) * 100, 2)
    return stock_price_end, performance
...
if __name__ == "__main__":
    ...
    ####### PAGE CONFIG ###########################
    st.set_page_config(
        page_title="udemy project screener",
        page_icon="📈",
    ...
    ####### MARGINS ###############################
    # Insert custom CSS into the application to set the margins.
    # This sets the margins for the entire page.
    ###############################################
    margins_css = """
    <style>
        .main > div {
    ...
    stock_price_end, performance = metrics(data_price)

    col_prediction_1, col_prediction_2 = st.columns([1, 2])

    with col_prediction_1:
        st.metric(
            label = "Stock Price on 2023-09-30", 
            value = str(np.around(stock_price_end, 2)), 
            delta = str(performance) + ' %')
        st.write('*Compared to 2013-09-30*')

    with col_prediction_2:
        with st.expander("Data Explanation", expanded=True):
            st.write("""
                The graph above shows the evolution of the selected stock price between 2013-09-30 and 2023-09-30.
                The indicator on the left is the stock price value in 2023-09-30 for the selected company and its evolution between 2013-09-30 and 2023-09-30.
